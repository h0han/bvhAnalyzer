...
class Bvh:
    ...
    def plot_hierarchy(self):
        ...
        right_arm_joints = [anim.joints['r_shoulder_JNT'], anim.joints['r_arm_JNT'], anim.joints['r_forearm_JNT']]
        right_arm_indices = [list(anim.joints.values()).index(joint) for joint in right_arm_joints]

        # plot all joints in blue color
        ax.scatter(pos[:, 0], pos[:, 2], pos[:, 1], c='b')

        # plot right arm joints in red color
        ax.scatter(pos[right_arm_indices, 0], pos[right_arm_indices, 2], pos[right_arm_indices, 1], c='r')
        # 요까지
    ...
    def _plot_pose(self, p, r, fig=None, ax=None):
        ...
        # 추가된 부분
        # find indices of right arm joints
        right_arm_joints = [anim.joints['l_handThumb3_JNT'], anim.joints['l_handThumb2_JNT'], anim.joints['l_handThumb1_JNT'],
                            anim.joints['l_hand_JNT'], anim.joints['l_forearm_JNT'],
                            anim.joints['l_arm_JNT'], anim.joints['l_shoulder_JNT']]
        right_arm_indices = [list(anim.joints.values()).index(joint) for joint in right_arm_joints]

        # # plot all joints in blue color
        ax.scatter(p[:, 0], p[:, 2], p[:, 1], c='w')

        # plot right arm joints in red color
        ax.scatter(p[right_arm_indices, 0], p[right_arm_indices, 2], p[right_arm_indices, 1], c='r')
        # 요까지
